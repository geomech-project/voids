"""Five-level 2D Brinkman manufactured-solution minimum working example."""
...
def _slope_triangle(
    ...
    *,
    ...
    """Draw a right triangle whose hypotenuse has ``rate`` on log-log axes."""
    ...
    text_x = np.sqrt(h_fine * h_coarse) if label_above else h_coarse * 1.12
...
def main(output_dir: Path) -> None:
    ...
    case = boundary_layer_case_2d(viscosity=viscosity, reaction=reaction)
    studies = {
    ...
    with (output_dir / "mms_2d_convergence.csv").open("w", newline="") as stream:
    ...
    velocity, pressure = case.evaluate(np.vstack((x_grid.ravel(), y_grid.ravel())))
    exact_fields = (
        velocity[0].reshape(x_grid.shape),
        velocity[1].reshape(x_grid.shape),
        pressure.reshape(x_grid.shape),
    )
    exact_labels = (r"$u_x$", r"$u_y$", r"$p$")
    ...
    figure.suptitle(rf"2D exact fields: $\nu={viscosity:g}$, $\gamma={reaction:g}$")
    figure.savefig(output_dir / "mms_2d_exact_fields.png", dpi=190)
    plt.close(figure)
    ...
    for axis, (metric, ylabel) in zip(axes, metrics, strict=True):
        ...
        for index, method in enumerate(("taylor_hood", "usfem_p1dg1")):
            ...
            _slope_triangle(
                ...
                y=(float(values[-1]) * 0.22 if index == 0 else float(finest_pair_peak) * 2.0),
                rate=rate,
                color=colors[method],
    ...
    figure.suptitle("2D Brinkman MMS — five mesh levels and measured finest-pair slopes")
    figure.savefig(output_dir / "mms_2d_convergence.png", dpi=190)
    plt.close(figure)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("examples/outputs/fem_mms/2d"),
    )
