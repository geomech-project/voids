...
def solve_tpfa(
    ...
    *,
    ...
    solver_method :
        ...
        ``"direct"``, ``"superlu"``, ``"pardiso"``, ``"nvmath_cudss"``,
        ``"cg"``, and ``"gmres"``.
    solver_parameters :
        ...
        For CPU backends that support runtime value precision selection, pass
        ``{"dtype": "float32"}`` or ``{"dtype": "float64"}``. For
        ``solver_method="nvmath_cudss"``, pass cuDSS controls such as
