...
`pnextract` + `pnflow` workflow. After instrumenting the checked-in `pnflow`
source, the benchmark is now interpreted in two layers:
...
2. full-workflow comparison, where `voids` re-extracts the binary image with
   `snow2` before solving.

That split matters scientifically. The first layer is now a same-network
software-verification test, while the second layer remains the end-to-end
workflow comparison against an independent external PNM pipeline.
...
3. the saved external reference built from `pnextract` network extraction plus
...
The `snow2` branch is still a workflow-level comparison. A mismatch there is
not automatically a `voids` bug, because the two sides differ in:
...
Even when both workflows are implemented correctly, the two benchmark branches
...
| Extraction backend | Reuse saved external extraction | `snow2` | `pnextract` |
...
| Case | `K_imported` [m^2] | `K_pnflow` [m^2] | Imported rel. diff. [%] | `K_snow2` [m^2] | `snow2` rel. diff. [%] |
|---|---:|---:|---:|---:|---:|
| `phi032_b14` | `9.752e-15` | `9.752e-15` | `0.000024` | `6.097e-15` | `37.48` |
| `phi035_b16` | `1.332e-14` | `1.332e-14` | `0.000031` | `1.702e-14` | `21.76` |
| `phi038_b18` | `1.199e-14` | `1.199e-14` | `0.000235` | `2.092e-14` | `42.67` |
| `phi040_b18` | `1.576e-14` | `1.576e-14` | `0.000301` | `2.033e-14` | `22.51` |
| `phi041_b20` | `1.437e-14` | `1.437e-14` | `0.000125` | `3.926e-14` | `63.40` |
...
- imported-CNM throat counts match the saved `pnflow` throat counts on all five
  cases
- the `snow2` branch still returns fewer pores and throats than the external
...
1. The `voids` single-phase solver, conduit conductance model, and pressure
   ...
   enabled.
2. The large remaining mismatch in the `snow2` branch is morphology-sensitive
   and dominated by extraction differences rather than the single-phase solver.
...
- the `snow2` branch remains an external workflow comparison between two
